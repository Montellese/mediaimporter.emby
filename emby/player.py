...
class Player(xbmc.Player):
    ...
    def _startPlayback(self):
        ...
        if not self._server or not self._server.Authenticate():
            Player.log('cannot connect to media provider {} to report playback progress of "{}" ({})' \
                .format(mediaProvider2str(mediaProvider), self._item.getLabel(), self._file), xbmc.LOGWARNING)
            self._reset()
            return
    ...
    def _addExternalSubtitles(self):
        ...
        itemUrl = self._server.BuildUserItemUrl(self._itemId)
        itemObj = self._server.ApiGet(itemUrl)
        if not itemObj:
